  ...
  <div id="app" class="flex container h-screen w-full">
    ...
    <div class="lg:w-1/5 border-r border-lighter px-2 lg:px-8 py-2 flex flex-col justify-between">
      <div>
        ...
        <div>
          <button v-for="tab in tabs" :key="tab.icon" @click="id = tab.id" :class="`focus:outline-none hover:text-blue flex items-center py-2 px-4 hover:bg-lightblue rounded-full mr-auto mb-3 ${ id === tab.id ? 'text-blue' : ''}`">
      ...
      <div class="lg:w-full relative">
        <button @click="dropdown = true" class="flex items-center w-full hover:bg-lightblue rounded-full p-2 focus:outline-none">
          <img src="profile.jpeg" class="w-10 h-10 rounded-full">
          <div class="hidden lg:block ml-4">
            <p class="text-sm font-bold leading-tight">Jared Watson</p>
            <p class="text-sm leading-tight">@jaredw</p>
          </div>
    ...
    <div class="w-1/2 h-full overflow-y-scroll">
      ...
      <div class="px-5 py-3 border-b-8 border-lighter flex">
        <div class="flex-none">
          <img src="profile.jpeg" class="w-12 h-12 rounded-full">
        </div>
        <form v-on:submit.prevent = "addNewTweet" class="w-full px-4 relative">
          <textarea v-model="tweet.content" placeholder="What's up?" class="w-full focus:outline-none mt-3 pb-3"></textarea>
      ...
      <div class="flex flex-col-reverse">
        <div v-for="tweet in tweets" :key="tweet.comment" class="w-full p-4 border-b hover:bg-lighter flex">
          <div class="flex-none mr-4">
            <img src="profile.jpeg" class="h-12 w-12 rounded-full flex-none"/>
          </div>
          <div class="w-full">
            <div class="flex items-center w-full">
              <p class="font-semibold"> Jared Watson </p>
              <p class="text-sm text-dark ml-2"> @jaredw </p>
      ...
      <div v-for="follow in following" :key="follow.src" class="w-full p-4 border-b hover:bg-lighter flex">
        <div class="flex-none mr-4">
    ...
    <div class="md:block hidden w-1/3 h-full border-l border-lighter py-2 px-6 overflow-y-scroll relative">
      ...
      <div class="w-full rounded-lg bg-lightest">
        ...
        <button v-for="trend in trending" :key="trend.top" class="w-full flex justify-between hover:bg-lighter p-3 border-t border-lighter focus:outline-none">
          <div>
      ...
      <div class="w-full rounded-lg bg-lightest my-4">
        ...
        <button v-for="friend in friends" :key="friend.src" class="w-full flex hover:bg-lighter p-3 border-t border-lighter focus:outline-none">
...
export default {
  ...
  data() {
    return {
      ...
      friends: [
        {src: 'elon.jpg', name: 'Elon Musk', handle: '@teslaBro'},
        {src: 'monk.jpg', name: 'Adrian Monk', handle: '@detective:)'},
        {src: 'kevin.jpg', name: 'Kevin Hart', handle: '@kev<3'}
      ],
      following: [
        {src: 'elon.jpg', name: 'Elon Musk', handle: '@teslaBro', time: '20 min', tweet: 'Should I just quarantine on mars??', comments: '1,000', retweets: '550', like: '1,000,003'},
        {src: 'kevin.jpg', name: 'Kevin Hart', handle: '@kev<3', time: '55 min', tweet: 'Should me and the rock do another sub-par movie together????', comments: '2,030', retweets: '50', like: '20,003'},
        {src: 'elon.jpg', name: 'Elon Musk', handle: '@teslaBro', time: '1.4 hr', tweet: 'Haha just made a flame thrower. Shld I sell them?', comments: '100,000', retweets: '1,000,002', like: '5,000,003'},
        {src: 'elon.jpg', name: 'Elon Musk', handle: '@teslaBro', time: '1.4 hr', tweet: 'Just did something crazyyyyyyy', comments: '100,500', retweets: '1,000,032', like: '5,000,103'}
      ],
      tweets: [
